...
public class QMatrix {
    ...
    public QMatrix(QVector ... rows) {
        this(rows.length, rows[0].getDimension(), (int row, int column) -> rows[row].get(column));
    }

    public QMatrix(Rational[] ... elements) {
        this(elements.length, elements[0].length, (int row, int column) -> elements[row][column]);
    }
    ...
    public QVector.View getRow(int row) {
        return new QVector.View(this.getColumnCount(), column -> this.get(row, column), (column, value) -> this.set(row, column, value));
    }

    public QVector.View getColumn(int column) {
        return new QVector.View(this.getRowCount(), row -> this.get(row, column), (row, value) -> this.set(row, column, value));
    }
    ...
    public QVector[] getColumnsCopy() {
        ...
        for (int i = 0; i < columns.length; ++i) {
            columns[i] = this.getColumnCopy(i);
        }
        return columns;
    }
    ...
    public QMatrix sub(int r1, int c1, int rowCount, int columnCount) {
        return new View(rowCount, columnCount, (row, column) -> this.get(r1 + row, c1 + column), (row, column, value) -> this.set(r1 + row, c1 + column, value));
    }
    ...
    public boolean equals(Object other) {
        ...
        for (int row = 0; row < this.getRowCount(); ++row) {
            for (int column = 0; column < this.getColumnCount(); ++column) {
                if (this.get(row, column).equals(matrix.get(row, column))) continue;
                return false;
            }
        }
        return true;
    ...
    public String toString() {
        ...
        QVector.View[] rows = this.getRows();
        for (int i = 0; i < rows.length; ++i) {
    ...
    public static interface Mapper {
        public Rational getNewValue(int var1, int var2, Rational var3);

        default public QVector.Mapper forRow(int row) {
            return (index, oldValue) -> this.getNewValue(row, index, oldValue);
        }

        default public QVector.Mapper forColumn(int column) {
    ...
    public static interface Generator {
        public Rational getValue(int var1, int var2);

        default public QVector.Generator forRow(int row) {
            return index -> this.getValue(row, index);
        }

        default public QVector.Generator forColumn(int column) {
            return index -> this.getValue(index, column);
        }

        default public Mapper asMapper() {
    ...
    extends QMatrix {
        ...
        @Override
        public Rational get(int row, int column) {
            return column < this.getSplit() ? this.getBaseMatrix().get(row, column) : this.getExtraMatrix().get(row, column - this.getSplit());
        }
    ...
    extends QMatrix {
        ...
        public static interface Setter {
            public void set(int var1, int var2, Rational var3);
        }
    }
}
